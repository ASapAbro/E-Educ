...
class Student:
    """Data-access helper for the 'etudiants' collection."""

    @staticmethod
    def collection():
        return get_db().etudiants

    @classmethod
    def create(cls, nom, prenom, email, niveau, statut="actif"):
        doc = {
            "nom": nom.strip(),
            "prenom": prenom.strip(),
    ...
    def update(cls, student_id, data):
        ...
        cls.collection().update_one({"_id": ObjectId(student_id)}, {"$set": data})

    @classmethod
    def delete(cls, student_id):
        cls.collection().delete_one({"_id": ObjectId(student_id)})

    @classmethod
    def find_all(cls, search=None, niveau=None, statut=None, sort_field="nom", sort_dir=1):
        query = {}
        if search:
    ...
    @classmethod
    def distinct_niveaux(cls):
        return sorted(cls.collection().distinct("niveau"))
